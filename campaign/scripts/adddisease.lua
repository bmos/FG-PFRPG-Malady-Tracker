...
function addDisease(nodeChar, sClass, sRecord, nodeTargetList)
	...
	if sClass == 'referencedisease' then
		...
		if DB.getChild(nodeSource, '....') and DB.getName(DB.getChild(nodeSource, '....')) ~= 'charsheet' then
		...
		if DB.getValue(nodeEntry, 'dc_notifier') == 1 then ChatManager.SystemMessage(Interface.getString('disease_msg_dcnotifier')) end
	else
		return false
	end
