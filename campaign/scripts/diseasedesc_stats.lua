...
local function ifLocked(sType)
	...
	if DB.getParent(getDatabaseNode()).getName() ~= 'disease' and DB.getChild(getDatabaseNode(), '...').getName() ~= 'reference' then
	...
	if sType == 'poison' then
		...
		if
			...
			and DB.getParent(getDatabaseNode()).getName() ~= 'disease'
			and DB.getChild(getDatabaseNode(), '...').getName() ~= 'reference'
		then
...
local function ifUnlocked(sType)
	...
	if sType ~= 'disease' then
		if DB.getParent(getDatabaseNode()).getName() == 'disease' or DB.getChild(getDatabaseNode(), '...').getName() == 'reference' then
