...
function notifyApplySave(rSource, rRoll)
    ...
    if DB.getValue(nodeDiseaseRoll, 'savesreq') ~= 0 then
        if rRoll.sSaveResult:match('failure') and
            DB.getValue(nodeDiseaseRoll, 'isconsecutive', 1) == 1 then
            DB.setValue(nodeDiseaseRoll, 'savecount_consec', 'number', 0)
        elseif rRoll.sSaveResult:match('success') then
            DB.setValue(nodeDiseaseRoll, 'savecount_consec', 'number',
                        DB.getValue(nodeDiseaseRoll, 'savecount_consec', 0) + 1)

            local nConsecutiveSaves = DB.getValue(nodeDiseaseRoll,
                                                  'savecount_consec', 0)
            ...
            if nSavesReq ~= 0 and nConsecutiveSaves >= nSavesReq then
                ...
                if not string.find(DB.getValue(nodeDiseaseRoll, 'name', ''),
                                   '%[CURED%]') then
                    DB.setValue(nodeDiseaseRoll, 'name', 'string',
                                '[CURED] ' .. sDiseaseName)
                end
            end
        end
    ...
    if rRoll.sSaveResult:match('failure') then
        ...
        local sPoisonEffect = DB.getValue(nodeDiseaseRoll,
                                          'poison_effect_primary', '')
        if sPoisonEffect ~= '' then
            sMaladyEffect =
                Interface.getString('disease_failure_effect_primary'):format(
                    sMaladyEffect, sPoisonEffect)
        end

        local sPoisonSecondary = DB.getValue(nodeDiseaseRoll,
                                             'poison_effect_secondary', '')
        if sPoisonSecondary ~= '' then
            sMaladyEffect = Interface.getString(
                                'disease_failure_effect_secondary'):format(
                                sMaladyEffect, sPoisonSecondary)
        end

        ChatManager.Message(
            Interface.getString('disease_failure_effect') .. ' ' ..
                sMaladyEffect, true, rSource)
    end
end
...
function getRoll(rActor, nodeDisease)
    ...
    if nodeActor then
        if ActorManager.isPC(rActor) then
            rRoll.nMod =
                DB.getValue(nodeActor, 'saves.' .. sSave .. '.total', 0)
    ...
    if sDiseaseType ~= '' then rRoll.tags = sDiseaseType .. 'tracker' end
...
function modSave(rSource, _, rRoll)
    ...
    local sSave
    if sSaveMatch then sSave = StringManager.trim(sSaveMatch):lower() end
    ...
    if rSource then
        ...
        if sModStat then sActionStat = DataCommon.ability_stol[sModStat] end
        if not sActionStat then sActionStat = guessSaveAbility(sSave) end

        -- Build save filter
        local aSaveFilter = {}
        if sSave then table.insert(aSaveFilter, sSave) end

        -- Determine flatfooted status
        local bFlatfooted = false
        if not rRoll.bVsSave and ModifierStack.getModifierKey('ATT_FF') then
            bFlatfooted = true
        elseif EffectManager35E.hasEffect(rSource, 'Flat-footed') or
            EffectManager35E.hasEffect(rSource, 'Flatfooted') then
            bFlatfooted = true
        end
        ...
        local aSaveEffects = EffectManager35E.getEffectsByType(rSource, 'SAVE',
                                                               aSaveFilter,
                                                               rSaveSource,
                                                               false)
        ...
        local nBonusStat, nBonusEffects =
            ActorManager35E.getAbilityEffectsBonus(rSource, sActionStat)
        ...
        local nNegLevelMod, nNegLevelCount =
            EffectManager35E.getEffectsBonus(rSource, {'NLVL'}, true)
        ...
        if bFlatfooted then table.insert(aAddDesc, '[FF]') end
...
function performRoll(draginfo, rActor, nodeDisease)
    ...
    if sDiseaseName and sDiseaseName ~= '' then
        rRoll.sDesc = rRoll.sDesc .. ' ' ..
                          string.format(Interface.getString('disease_against'),
                                        sDiseaseName)
    end
