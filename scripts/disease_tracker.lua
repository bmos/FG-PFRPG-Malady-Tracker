...
function parseDiseases()
    ...
    for _, nodeCT in pairs(DB.getChildren(CombatManager.CT_LIST)) do
        ...
        for _, nodeDisease in pairs(DB.getChildren(nodeActor, 'diseases')) do
            local nDateOfContr = DB.getValue(nodeDisease, 'starttimestring') or
                                     DB.getValue(nodeDisease, 'starttime')
            if nDateOfContr and DB.getValue(nodeDisease, 'savetype') and
                not string.find(DB.getValue(nodeDisease, 'name', ''), '%[') then
                ...
                local sDiseaseName = DB.getValue(nodeDisease, 'name',
                                                 'their disease')
                Debug.console(sDiseaseName,
                              'nDiseaseElapsed: ' .. nDiseaseElapsed)
                ...
                if nDiseaseElapsed > 0 then
                    ...
                    if nFreq then
                        local nPrevRollCount =
                            DB.getValue(nodeDisease, 'savecount', 0) -- how many saves have been successful
                        ...
                        if nTargetRollCount > 0 then
                            local nDurationWithOnset, nDuration, _ =
                                calculateDuration(nodeDisease, nOnset)
                            if nDurationWithOnset then
                                nTargetRollCount =
                                    math.min(nTargetRollCount, nDuration / nFreq);
                            end
                            ...
                            local bIsAutoRoll =
                                DB.getValue(nodeActor, 'diseaserollactive', 1) ==
                                    1
                            local nRollCount = 0
                            if not bIsAutoRoll and nTargetRollCount > 0 then
                                ChatManager.Message(string.format(
                                                        Interface.getString(
                                                            'disease_agencynotifer'),
                                                        DB.getValue(nodeActor,
                                                                    'name',
                                                                    'A character'),
                                                        nTargetRollCount,
                                                        'a ' ..
                                ...
                                repeat
                                    ActionDiseaseSave.performRoll(nil, rActor,
                                                                  nodeDisease)
                            ...
                            if (nDuration and nDuration > 0) and
                                (nDiseaseElapsed >= nDuration) then
                                DB.setValue(nodeDisease, 'starttime', 'number',
                                            nil)
                                DB.setValue(nodeDisease, 'starttimestring',
                                            'string', nil)
                                DB.setValue(nodeDisease, 'savecount', 'number',
                                            nil)
                                if not string.find(
                                    DB.getValue(nodeDisease, 'name', ''),
                                    '%[EXPIRED%]') then
                                    DB.setValue(nodeDisease, 'name', 'string',
                                                '[EXPIRED] ' .. sDiseaseName)
                                end
                                ChatManager.Message(string.format(
                                                        Interface.getString(
                                                            'disease_expiration'),
